...
contract TicTacPonzi {
    ...
    */
    ...
    in the pot
    */
    function TicTacPonzi() hasValue gameInactive {
        clear(msg.sender)
    }
    ...
    */
    function challenge() has_value {
        if (msg.sender != payee) && (current_game.challenger == 0) && 
            (current_game.active == false) && (current_game.players == 1) {
    ...
    */
    function getPlayerToken(address player) returns (uint token) {
    ...
    Returns 0 if there is an error.
    */
    function getPlayerAddress(uint token) returns (address player) {
        if (token == 1) {
    ...
    */
    function checkWin(uint player_token) returns (bool win_check){
    ...
    */
    function timeOut(address victor) {
        // mechanism to check time.
        // winner(victor);
    }
    ...
    */
    function winner(address victor) {
    ...
    */
    function newBoard(address host) {
    ...
    '-1'.
    */
    function play(uint row, uint column) {
    ...
    has made so far.
    */
    function getState(address host) returns (uint _pot, uint _turn,
